...
def test_loaded_input_package_projects_rows_from_generated_assumptions() -> None:
    ...
    assert package.capital_position_for("BANKING_BOOK", package.forecast_calendar[0].as_of_date)
    assert len(package.operational_losses_for("BANKING_BOOK")) == 10
    assert package.cva_netting_sets_for("BANKING_BOOK")
    assert package.leverage_off_balance_sheet_items_for("BANKING_BOOK")
