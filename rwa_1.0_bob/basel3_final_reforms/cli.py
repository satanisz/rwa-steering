...
def run_batch_calculation(
    ...
    nccr_mapping_path: Optional[str] = None,
    verbose: bool = False
) -> None:
    """
    ...
    core_info_list = load_csv_to_records(
        core_info_path,
        CoreInfoRecord,
        CORE_INFO_COLUMNS
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Basel III RWA Calculator - Batch Processing CLI"
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to core info CSV file"
    )
    ...
    parser.add_argument(
        "--nccr-mapping",
        type=str,
        help="Path to NCCR mapping CSV file (default: nccr_mapping.csv)"
    )
    ...
    try:
        run_batch_calculation(
            ...
            nccr_mapping_path=args.nccr_mapping,
