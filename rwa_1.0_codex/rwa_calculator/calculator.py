...
class RwaCalculator:
    def __init__(
        self,
        nccr_mapping_path: str | Path = "nccr_mapping.csv",
    ...
    def from_files(
        cls,
        nccr_mapping_path: str | Path = "nccr_mapping.csv",
        country_info_path: str | Path = "preprod_country_info.csv",
    ) -> "RwaCalculator":
        return cls(nccr_mapping_path=nccr_mapping_path, countries=load_country_info(country_info_path))
    ...
    def calculate_batch(
        ...
        for index, row in enumerate(rows, start=1):
            ...
            except Exception as exc:  # noqa: BLE001 - API must collect row-level validation errors.
    ...
    def calculate_record(self, record: CoreInfoRecord) -> tuple[dict[str, Any], list[CalculationTraceStep]]:
        ...
        trace.append(
            CalculationTraceStep(
                step_id="pd_lookup",
                description="Lookup PD from NCCR/CRR mapping using local-currency grade when exposure currency matches country local currency.",
                input_values={
        ...
        trace.append(
            CalculationTraceStep(
                ...
                description="Determine Basel 3.0 DLGD, Basel 3.1 foundation LGD and effective maturity.",
                input_values={
                ...
                rule_reference={"source_document": "Basel III Finalising post-crisis reforms", "section": "7.6"},
            )
        )

        basel_3_0_rw = self._irb_risk_weight(record, basel_3_0_pd, basel_3_0_dlgd, maturity)
        basel_3_1_foundation_rw = self._irb_risk_weight(record, basel_3_1_pd, basel_3_1_dlgd, maturity)
        ...
        trace.append(
            CalculationTraceStep(
                step_id="risk_weight",
                description="Calculate IRB foundation, standardised and exposure-level output-floor proxy risk weights.",
                input_values={
                    "avc": str(record.avc),
                    "entity_class": record.entity_class,
                ...
                rule_reference={"source_document": "Basel III Finalising post-crisis reforms", "section": "7.3/10"},
            )
        )
    ...
    def _standardised_risk_weight(self, record: CoreInfoRecord, country: CountryInfoRecord) -> Decimal:
        rating = record.trade_external_rating or record.counterparty_external_rating
        if record.entity_class == "SOV":
            rw = sovereign_external_rw(record.counterparty_external_rating or country.country_external_rating)
            if (
        ...
        if record.entity_class in {"BANK", "FI"}:
            ...
            return ecra if ecra is not None else bank_scra_rw(record.counterparty_credit_quality_grade, short_term)
        if record.entity_class == "RETAIL":
            return Decimal("0.75")
        if record.entity_class == "OTHER":
            return Decimal("1.00")
        if record.sub_class in {"OBJECT_FINANCE", "COMMODITIES_FINANCE"} and record.trade_external_rating is None:
            return Decimal("1.00")
        ...
        if record.sub_class == "COMMERCIAL_REAL_ESTATE":
            return Decimal("1.00")
        return corporate_external_rw(rating, record.counterparty_credit_quality_grade == "INVESTMENT_GRADE")
    ...
    def _irb_risk_weight(
        ...
        if record.entity_class in {"RETAIL"}:
            ...
            return self._standardised_risk_weight(record, self.countries[record.incorporation_country])
        else:
            k = self._corporate_bank_capital_requirement(pd, lgd, maturity)
        return max(Decimal("0"), Decimal("12.5") * k * record.avc)

    def _corporate_bank_capital_requirement(self, pd: Decimal, lgd: Decimal, maturity: Decimal) -> Decimal:
    ...
    def _retail_capital_requirement(self, record: CoreInfoRecord, pd: Decimal, lgd: Decimal) -> Decimal:
        pd_f = min(max(float(pd), 0.000001), 0.999999)
        lgd_f = float(lgd)
        if record.sub_class == "RESIDENTIAL_REAL_ESTATE":
            correlation = 0.15
    ...
    def _build_projection(self, result: dict[str, Any], projection_date: str) -> dict[str, Any]:
        fields = {
            ...
            if key in result and key not in {"counterparty_gid", "basel_3_0_pd", "basel_3_1_pd", "basel_3_0_dlgd", "basel_3_1_dlgd", "basel_3_1_rw_final", "basel_3_1_rwa_final", "basel_3_1_ro_rw_final"}
        }
