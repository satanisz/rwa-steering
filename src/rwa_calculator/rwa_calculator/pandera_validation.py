...
CORE_INFO_SCHEMA = pa.DataFrameSchema(
    {
        ...
        "counterparty_fcy_internal_rating": pa.Column(
            str, _isin(NCCR_RATING_GRADES), nullable=False
        ),
        "counterparty_lcy_internal_rating": pa.Column(
            str, _isin(NCCR_RATING_GRADES), nullable=False
        ),
        "incorporation_country": pa.Column(
            str, pa.Check.str_matches(r"^[A-Z]{2}$"), nullable=False
        ),
...
COUNTRY_INFO_SCHEMA = pa.DataFrameSchema(
    {
        "incorporation_country": pa.Column(
            str, pa.Check.str_matches(r"^[A-Z]{2}$"), nullable=False, unique=True
        ),
        "local_currency": pa.Column(str, pa.Check.str_matches(r"^[A-Z]{3}$"), nullable=False),
        "country_dlgd": pa.Column(
            float, [pa.Check.ge(0), pa.Check.le(1)], nullable=True, coerce=True
        ),
