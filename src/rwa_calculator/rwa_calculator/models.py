...
class CoreInfoRecord:
    ...
    def from_mapping(cls, row: dict[str, Any]) -> CoreInfoRecord:
        ...
        if credit_quality not in COUNTERPARTY_CREDIT_QUALITY_GRADES:
            raise ValueError(
                f"counterparty_credit_quality_grade has unsupported value {credit_quality!r}"
            )
        ...
        record = cls(
            ...
            incorporation_country=require_code(
                row["incorporation_country"], "incorporation_country", 2
            ),
    ...
    def from_mapping(cls, row: dict[str, Any]) -> CountryInfoRecord:
        ...
        record = cls(
            ...
            country_internal_rating=(
                ...
                else normalise_rating_grade(
                    row["country_internal_rating"], "country_internal_rating"
                )
            ),
            country_external_rating=normalise_optional_external_rating(
                row["country_external_rating"], "country_external_rating"
            ),
            sov_concessionary_flag=require_flag(
                row["sov_concessionary_flag"], "sov_concessionary_flag"
            ),
        )
        if record.country_dlgd is not None and not Decimal("0") <= record.country_dlgd <= Decimal(
            "1"
        ):
