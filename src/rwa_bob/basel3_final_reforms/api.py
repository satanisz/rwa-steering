...
app = FastAPI(
    ...
    description=(
        "REST API for calculating Risk-Weighted Assets according to Basel III final reforms"
    ),
    version="0.1.0",
)

# Global engine instance
_engine: RwaEngine | None = None
...
async def calculate_rwa(request: RwaCalculationRequest) -> RwaCalculationResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Calculation error: {e!s}")
...
async def calculate_rwa_with_trace(request: RwaCalculationRequest):
    ...
    try:
        engine = get_engine()
        return engine.calculate_with_trace(request.core_info, request.country_info)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Calculation error: {e!s}")
