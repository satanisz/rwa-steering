...
from rwa_bob.paths import NCCR_MAPPING_PATH


class ReferenceDataLoader:
    """Loads and manages reference data for RWA calculations."""

    def __init__(self, nccr_mapping_path: str | None = None):
        """
        ...
        """
        self.nccr_mapping_path = Path(nccr_mapping_path) if nccr_mapping_path else NCCR_MAPPING_PATH
    ...
    def get_pd_for_rating(self, rating_grade: str, entity_bucket: str) -> Decimal | None:
        """
    ...
    def get_sovereign_risk_weight(self, external_rating: str | None) -> Decimal:
        """
        ...
        if rating_upper in ["AAA", "AA+", "AA", "AA-"]:
            return Decimal("0.0")
        # A+ to A-: 20%
        if rating_upper in ["A+", "A", "A-"]:
            return Decimal("0.2")
        # BBB+ to BBB-: 50%
        if rating_upper in ["BBB+", "BBB", "BBB-"]:
            return Decimal("0.5")
        # BB+ to B-: 100%
        if rating_upper in ["BB+", "BB", "BB-", "B+", "B", "B-"]:
            return Decimal("1.0")
        # Below B-: 150%
        return Decimal("1.5")

    def get_bank_risk_weight_ecra(
        self, external_rating: str | None, is_short_term: bool = False
    ) -> Decimal:
        """
    ...
    def get_corporate_risk_weight(self, external_rating: str | None) -> Decimal:
        """
    ...
    def get_pse_risk_weight(
        self, sovereign_rating: str | None, pse_rating: str | None, use_option_2: bool = False
    ) -> Decimal:
        """
        ...
        if use_option_2 and pse_rating:
            ...
            if rating_upper in ["AAA", "AA+", "AA", "AA-"]:
                return Decimal("0.2")
            if rating_upper in ["A+", "A", "A-"] or rating_upper in ["BBB+", "BBB", "BBB-"]:
                return Decimal("0.5")
            if rating_upper in ["BB+", "BB", "BB-", "B+", "B", "B-"]:
                return Decimal("1.0")
            if pse_rating == "UNRATED":
                return Decimal("0.5")
            return Decimal("1.5")
    ...
    def get_mdb_risk_weight(
        self, is_eligible_zero_rw: bool, external_rating: str | None = None
    ) -> Decimal:
        """
