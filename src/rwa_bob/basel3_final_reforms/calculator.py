...
class RwaCalculator:
    ...
    def _get_entity_bucket(self, entity_class: EntityClass) -> str:
        ...
        if entity_class in {EntityClass.SOV, EntityClass.PSE, EntityClass.MDB}:
            return "SOV"
        if entity_class in {EntityClass.BANK, EntityClass.FI}:
            return "BANK"
        return "CORP"
    ...
    def _calculate_risk_weight(
        self, core_info: CoreInfoRecord, country_info: CountryInfoRecord | None
        ...
        if core_info.avc != Decimal("1.0"):
            ...
            rw = rw * core_info.avc
            steps.append(
                f"Applied AVC multiplier {core_info.avc}: {original_rw * 100}% → {rw * 100}%"
            )
    ...
    def calculate_exposure_rwa(
        ...
        steps.append(f"RWA calculation: {exposure_amount} x {risk_weight * 100}% = {rwa_basel_3_0}")
    ...
    def calculate_portfolio_rwa(
        self, core_info_list: list[CoreInfoRecord], country_info_list: list[CountryInfoRecord]
        ...
        country_lookup = {country.incorporation_country: country for country in country_info_list}
        ...
        for core_info in core_info_list:
            try:
                ...
                error_results.append(
                    {"id": core_info.id, "messages": [f"Calculation error: {e!s}"]}
                )
